...
let carYears = [];
    for (let i = 0; i < inventory.length; i++) {
      let currentObj = inventory[i];
      carYears.push(currentObj.car_year);
    }
...
let oldCars = [];
  for (let i = 0; i < carYears.length; i++) {
    let currentObj = carYears[i];
    if (currentObj < 2000) {
      oldCars.push(currentObj);
    } 
...
let BMWAndAudi = [];
  for (let i = 0; i < inventory.length; i++) {
    let currentObj = inventory[i];
    if (currentObj.car_make === 'Audi' || currentObj.car_make === 'BMW') {
      BMWAndAudi.push(currentObj);
    }
  }
